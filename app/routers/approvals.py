...
from app.workflow import (
    ...
    get_booking_nodes,
    get_change_current_node,
    get_change_nodes,
    has_booking_finished_all_nodes,
    has_change_finished_all_nodes,
...
def _enrich_change(
    ...
    from app.routers.booking_changes import _enrich_change_response
    try:
        return _enrich_change_response(change, db)
    except Exception:
        return _enrich_change_response(change, None)


def _apply_booking_update_on_approval(db: Session, change: BookingChangeApplication):
    booking = db.query(BookingApplication).filter(
        BookingApplication.id == change.booking_id
    ).first()
    if not booking:
        return
    booking.booking_date = change.target_booking_date
    booking.start_time = change.target_start_time
    booking.end_time = change.target_end_time
    booking.attendees = change.target_attendees
    booking.purpose = change.target_purpose
...
@router.get("/changes/pending", response_model=BookingChangeListResponse)
def list_pending_change_approvals(
    skip: int = 0,
    limit: int = 100,
    db: Session = Depends(get_db),
    current_user: User = Depends(require_admin_or_auditor),
    ...
    query = query.order_by(BookingChangeApplication.submitted_at.asc())
    total = query.count()
    changes = query.offset(skip).limit(limit).all()
    enriched = [_enrich_change(c, db) for c in changes]
    return BookingChangeListResponse(items=enriched, total=total)


@router.get("/changes/{change_id}/workflow", response_model=BookingChangeResponse)
def get_change_approval_workflow(
    change_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(require_all_authenticated),
):
    ...
    return _enrich_change(change, db)


@router.get("/changes/{change_id}/records", response_model=List[ApprovalRecordResponse])
def list_change_approval_records(
    change_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(require_all_authenticated),
):
    ...
    records = db.query(ApprovalRecord).filter(
        ApprovalRecord.change_id == change_id,
...
@router.post("/changes/{change_id}/action", response_model=BookingChangeResponse)
def perform_change_approval_action(
    change_id: int,
    action_req: ApprovalActionRequest,
    db: Session = Depends(get_db),
    current_user: User = Depends(require_admin_or_auditor),
    ...
    if change.status not in [BookingStatus.PENDING, BookingStatus.APPROVING]:
        raise HTTPException(
            status_code=400,
            detail=f"当前状态为 {change.status.value}，无法执行审批操作",
        )

    nodes = get_change_nodes(db, change)
    total_nodes = len(nodes)
    if total_nodes == 0:
        raise HTTPException(status_code=400, detail="该变更申请未配置审批流程")

    if has_change_finished_all_nodes(db, change):
        change.status = BookingStatus.APPROVED
        _apply_booking_update_on_approval(db, change)
        db.commit()
        db.refresh(change)
        return _enrich_change(change, db)
    ...
    if change.current_node_index >= total_nodes:
        if change.workflow_snapshot:
            raise HTTPException(
                status_code=400,
                detail=f"该变更申请快照共 {total_nodes} 个节点，当前节点索引 {change.current_node_index} 超出范围，请检查数据完整性",
            )
        else:
            change.status = BookingStatus.APPROVED
            _apply_booking_update_on_approval(db, change)
    ...
    current_node = get_change_current_node(db, change)
    if not current_node:
        raise HTTPException(status_code=400, detail="无法定位当前审批节点")
    ...
    record = ApprovalRecord(
        ...
        change_id=change.id,
        change_type="change",
        node_index=change.current_node_index,
        node_name=current_node.node_name,
        auditor_id=current_user.id,
        action=action_req.action,
        comment=action_req.comment,
    ...
    if action_req.action == ApprovalAction.AGREE:
        next_index = change.current_node_index + 1
        if next_index >= total_nodes:
            change.status = BookingStatus.APPROVED
            change.current_node_index = next_index
            _apply_booking_update_on_approval(db, change)
        else:
            change.status = BookingStatus.APPROVING
            change.current_node_index = next_index

    elif action_req.action == ApprovalAction.RETURN:
        change.status = BookingStatus.RETURNED

    elif action_req.action == ApprovalAction.TERMINATE:
        change.status = BookingStatus.TERMINATED
...
@router.post("/changes/{change_id}/timeout-check", response_model=MessageResponse)
def check_and_handle_change_timeout(
    change_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(require_admin_or_auditor),
):
    ...
    if change.status not in [BookingStatus.PENDING, BookingStatus.APPROVING]:
        return MessageResponse(message=f"当前状态 {change.status.value} 无需超时检查")

    nodes = get_change_nodes(db, change)
    total_nodes = len(nodes)
    if total_nodes == 0 or change.current_node_index >= total_nodes:
        return MessageResponse(message="无待处理节点")
    ...
    start_time = last_record.created_at if last_record else change.submitted_at
    elapsed_minutes = (datetime.utcnow() - start_time).total_seconds() / 60
    timeout_minutes = current_node.timeout_minutes
    ...
    if strategy == TimeoutStrategy.REMIND:
        ...
        record = ApprovalRecord(
            ...
            auditor_id=current_user.id,
            action=ApprovalAction.TERMINATE,
            comment=f"[系统超时驳回] 节点 '{current_node.node_name}' 审批超时，自动终止变更申请",
        )
        db.add(record)
        change.status = BookingStatus.TERMINATED
        db.commit()
        return MessageResponse(
            message=f"[超时驳回] 节点 '{current_node.node_name}' 审批超时，变更申请已自动终止"
        )
